...
class PathPlannerNode(Node):
    def __init__(self):
        ...
        # Cells to pad around obstacles. Was 5 (1.0m, ~vehicle half-width),
        # then 9 (1.8m) to keep canopy-overhang obstacles outside the ACC's
        # 0.6m on-path LiDAR check (see autonomous_cruise_node.py). Still not
        # enough: dilation expands the blocked zone in *every* direction
        # (including forward, along the approach), and that's also what
        # determines how early the A* path starts diverging from centerline.
        # At 1.8m the path was committing to the lateral move too late and
        # clipping the obstacle before finishing the turn. Raised further so
        # the detour starts sooner and has enough distance to complete before
        # reaching the obstacle.
        self.obstacle_padding = 16  # 3.2m
